...
## 2026-05-06 20:30 · Raven MBA M4 · 中英双语 i18n 实现
...
**做了什么**:
1. 实现完整中英双语国际化系统（i18n.py + zh.json + en.json）
2. 238 条翻译 key，覆盖所有用户可见文本（GUI、CLI、renderer、HTML viewer、日志）
3. 系统语言自动检测（macOS AppleLocale / Windows LCID / fallback locale）
4. 语言偏好持久化到备份目录 `language_preference`（与备份数据绑定）
5. GUI 右下角语言切换按钮，点击即时重建 UI
6. 备份/导出完成时自动写入语言偏好
7. HTML 查看器：注入 I18N JSON 对象，JS 动态读取翻译文本，`toLocaleDateString` 使用动态 locale
8. 构建脚本更新：`build-mac.sh` / `build-win.bat` 添加 locales 打包 + i18n hidden import
9. 修复 latent bug：`cli_exporter.py` 中 `log` 变量未定义
10. 修复测试：`test_safe_name` 需先 `load_locale("zh")`

**观察到的现象**:
- `html_viewer.py` 的 JS 代码中 ~25 处硬编码中文全部替换为 I18N 引用
- `_parse_content_blocks` 中 knowledge/file placeholder 的括号需独立处理（locale 值不含括号，代码加 `[{}]` 包裹）
- `_convert_cli_events` 的 summary placeholder 在数据采集时使用 `_()`，嵌入 JSON 时已翻译
- 工具返回的 `content` 中包含 `[图片]` — 已通过 `html.image_placeholder` 统一翻译
- `cookies.py` 的 `file.download_fail` locale 去掉 "HTTP" 前缀以兼容非 HTTP 异常

**验证**:
- smoke test 8/8 通过 ✓
- 所有模块语法编译通过 ✓
- 238 条 key zh ↔ en 完全一致 ✓
- `python3 -c "load_locale('en'); print(_('gui.title'))"` → "ClaudeDataBackup" ✓
- `python3 -c "load_locale('zh'); print(_('html.viewer_title'))"` → "ClaudeDataBackup 聊天记录查看器" ✓

**结论**:
- i18n 基础设施完整，可投入实际测试
- 待做：Mac GUI 实际运行验证（中英切换、备份流程、HTML 查看器）、Windows 端完整验证
- 保守选择：`log.py` 的 `_log_environment()` 在 `setup_logging()` 中调用，可能在 `init_language()` 之前执行。此时 `_()` 返回 key 名而非翻译（"log.startup" 而非 "ClaudeDataBackup v{ver} 启动"）。如需修复，调整 main.py/gui.py 的初始化顺序即可

---

## 2026-05-07 22:30 · Raven Mac · v0.2.0 GUI 滚动架构重构 + bug 修复

**环境**：
- macOS 26.4.1，Apple Silicon M4
- Python 3.12.13，customtkinter 5.2，PyInstaller 6.20
- Claude Desktop 已登录（sessionKey 可用）

**做了什么**：

1. **macOS 触控板滚动根因诊断**（6 轮 + 3 轮诊断脚本验证）：
   - 确认 macOS Tk/Cocoa 不对 Canvas-based 控件生成 `<MouseWheel>` 事件
   - customtkinter 全系控件（CTkFrame、CTkButton、CTkScrollableFrame 等）均基于 CTkCanvas，全部不接收触控板事件
   - Text widget 通过 NSScrollView 在 OS 层面原生支持，完全不经过 Tk 事件系统
   - 删除 6 个诊断脚本（`_scroll_diag.py` ~ `_scroll_diag6.py`）

2. **GUI 架构重构**：
   - 去掉滚动容器（Text container + CTkScrollbar + `_bind_scroll()`），改为固定布局
   - 主窗口 580×720，`grid_rowconfigure(weight=1)` 让日志区填充剩余空间
   - 日志区：CTkTextbox（原生触控板滚动）
   - CC 项目列表：CTkScrollableFrame → CTkTextbox + 文本勾选标记（☑/☐）
   - CTkTextbox 的 `height` 参数是像素不是行数——初始 130px，动态 100-200px
   - 勾选标记颜色：已选蓝（accent 色），未选灰
   - `_refresh_project_list()` + `_on_project_click()` 实现点击行 toggle BooleanVar
   - `wrap="word"` 自动换行长项目名

3. **Bug 修复**：
   - `cookies.py:207,219`：`for _, _, enc in target` 遮蔽 i18n `_` 导入 → `UnboundLocalError`。改为 `for _h, _n, enc in target`
   - `main.py:405`：`_incremental_mode_c` 使用未定义 `backup_dir` → `NameError`。添加 `backup_dir: Path` 参数
   - CTkTextbox `_textwidget` → `_textbox`（属性名错误导致闪退）

4. **按钮颜色统一**：Apple 蓝色系（#007AFF / #0A84FF），accent/hover/secondary 一致

**验证**：
- smoke test 8/8 通过 ✓
- 双击 .app：窗口正常、日志区触控板滚动正常、CC 项目列表触控板滚动正常 ✓
- 中/英切换按钮正常 ✓
- DMG 构建成功：`ClaudeDataBackup-v0.2.0-arm64.dmg`（17.5MB）✓

**结论**：
- macOS GUI 触控板滚动彻底解决，核心方案：禁止在 macOS 上用 Canvas-based 控件做滚动内容
- 待：Windows exe 打包验证（Win 机器上跑 build-win.bat）

---

## 2026-05-08 03:00~03:15 · 小桃子 Windows · exe 打包 + i18n 初始化修复

**环境**：
- Windows 11 Pro (Build 26200)
- Python 3.12.10 + venv（重建，指向 D:\claudeDataBackup）
- PyInstaller 6.20.0
- Claude Desktop 运行中（sessionKey 可用，16 条对话）

**做了什么**：
1. 环境验证：venv 重建，pip install -e ".[dev]" 成功
2. Smoke test：8/8 通过
3. CLI 全模式验证：
   - Mode A：cookie 解密成功，16 条对话在线获取，增量 2 条新增
   - Mode B：3 条缓存对话，无报错
   - Mode C：6 个 real session，`backup_dir` bug 修复生效
4. PyInstaller 打包：`dist/ClaudeDataBackup.exe`（25 MB），构建成功
5. **i18n 启动日志修复**：
   - 问题：`log.py` 的 `_log_environment()` 在 `init_language()` 前执行，日志显示 raw key（`log.startup`）而非翻译
   - 修复：`main.py`、`gui.py`、`run_gui.py` 三个入口点调整顺序，`init_language()` → `setup_logging()`
   - 验证：日志正确显示 `ClaudeDataBackup v0.2.0 启动` / `平台: Windows 11` 等翻译后文本

**观察到的现象**：
- Mode A cookie 解密正常（131 字符 sessionKey），API 抓取全部 16 条对话成功
- `backup_dir` NameError bug 确认修复，Mode C 增量备份无闪退
- PyInstaller 构建无 warning（除已知的 darkdetect AppKit framework ctypes 忽略）
- i18n 修复后日志环境信息全部正确翻译

**结论 / 下一步**：
- Windows exe 打包完成，CLI 全模式验证通过
- 待用户做 GUI 验证（双击 exe、中英切换、备份流程、HTML 查看器）
- 待上传 exe 到 v0.2.0 release

---

## 2026-05-18 01:00~03:00 · Raven Mac · v0.3.0 自动备份完整开发 + 自测

**环境**：
- macOS 26.4.1，MacBook Air M4
- Python 3.12.13 + venv
- Claude Desktop 1.4758.0（Electron 41.3.0 / Chrome 146.0.7680.188）
- Claude Code 2.1.143

**做了什么**：

### 自动备份调度引擎
- `scheduler.py`：ScheduleConfig / TimeTrigger / ConditionTriggers dataclass，evaluate_schedule() 统一触发评估，get_next_run_time() GUI 展示
- 支持 periodic / daily / weekly / monthly 四种时间触发 + 三种条件触发（Claude 启动/关闭/系统唤醒）
- compound logic：min_interval_minutes 对所有触发类型生效（manual 除外）
- schedule.json 从 `~/.claude-data-backup/` 移至 `<backup_dir>/`（与备份数据绑定）

### macOS 执行层
- `scheduler_mac.py`：launchd plist 管理，数组 ProgramArguments（不经过 /bin/sh），KeepAlive + RunAtLoad
- `autobackup_daemon.py`：常驻守护进程，内部定时器 + Claude 进程轮询 + 状态文件写盘
- macOS 菜单栏图标：PyObjC NSStatusBar "CB" 图标 + 菜单（立即备份/打开设置/退出）
- 系统通知：osascript display notification

### GUI
- 自动备份卡片：checkbox 触发器（定时 + 关闭 Claude）+ H+M 时间选择器
- daemon 状态显示：⟳ 正在启动 / ● 运行中 / ○ 已停止 / ✗ 启动失败
- 首次备份 gate：未备份过时 checkbox 灰色不可选
- 窗口 580×860，日志 textbox height=14
- 启动时自动拉起 daemon（延迟 2.5s，安全检查）

### 踩坑记录
- PyInstaller bootloader 不支持 `-m`：创建 `run_daemon.py` 入口脚本替代
- PyInstaller bootloader 作为 launchd daemon python → 无限打开 GUI 窗口：`_daemon_python()` 排除 bootloader，强制用真 Python
- launchd cwd=/ → editable install 找不到包：`run_daemon.py` 显式 `sys.path.insert(0, src/)`
- `min_interval_hours` → `min_interval_minutes` 重命名：多处遗漏引用导致 AttributeError 闪退
- status 被 `_poll_auto_status` 覆写：缓存 `_schedule_config` 避免重新 load

### 自测结果
- Smoke test 8/8
- 10 个模块全部可导入
- Scheduler config save/load/backward-compat 通过
- evaluate_schedule 逻辑正确（time/manual/claude_close/disabled）
- Daemon 生命周期正常（启动/检测/停止/状态文件）
- CLI --schedule status/uninstall 正常
- Mode B+C 增量备份正常（40 conversations + 54 CLI sessions）
- 通知系统正常
- 版本号 0.3.0-dev

**结论 / 下一步**：
- Mac 端 v0.3.0 开发 + 自测完成
- 项目移交 Windows（小桃子）做全功能验证
- 交接文档：`docs/handoff.md`

---

## 2026-05-17 23:00~00:30 · Raven Mac · 兼容性验证 + 动态 UA 检测

**环境**：
- macOS 26.4.1，MacBook Air M4
- Python 3.12.13 + venv
- Claude Desktop 1.4758.0（Electron 41.3.0 / Chrome 146.0.7680.188）— 账号被封，无法登录
- Claude Code 2.1.143
- 项目 v0.2.0 → v0.3.0-dev

**做了什么**：
1. **版本跨度调查**：
   - Claude Desktop：`0.14.2` → `1.4758.0`（大版本号体系变更）
   - Electron：`30.0.0` → `41.3.0`（+11 大版本）
   - Chrome：`124.0.0.0` → `146.0.7680.188`（+22 大版本）
   - Claude Code：`2.1.114` → `2.1.143`（+29 小版本）
2. **数据结构兼容性验证**：
   - Cookie 格式：v10 AES-GCM 不变，schema 20 列一致
   - Cache 格式：Simple Cache v5 不变，magic `0xFCFB6D1BA7725C30` 不变
   - Claude Code JSONL：schema 兼容，type/uuid/sessionId/message 结构不变
3. **功能验证**：
   - cookies.py：19 条 cookie 检测正常（sessionKey 缺失 — 预期，账号已封）
   - cache_extractor.py：40 条对话恢复，与旧版结果一致
   - cli_exporter.py：54 real + 1551 observer + 627 test，正常
   - Mode B+C 增量备份：40 桌面对话 + 54 CLI session，HTML 135 条对话 ~106MB
   - Smoke test：8/8 通过
4. **动态 User-Agent 检测实现**：
   - `paths.py`：`detect_claude_desktop_info()` — Mac mmap 扫 Framework 二进制 + plistlib 读版本，Win pywin32 + PowerShell 兜底
   - `paths.py`：`get_user_agent()` — 构造完整 UA，Electron→Chrome 映射表兜底（已知 30-41）
   - `api_fetcher.py` / `file_extractor.py`：硬编码 UA → `paths.get_user_agent()`
   - 验证：`claudeai/1.4758.0 Chrome/146.0.7680.188 Electron/41.3.0` 正确
5. **文档**：
   - CHANGELOG.md 创建
   - STATUS.md / testing-log.md 更新
   - docs/handoff.md 重写（Mac→Win Mode A 验证交接）

**观察到的现象**：
- Claude Desktop 版本体系从 `0.x` 跳到 `1.4758.0`，但底层 Chromium 数据结构（cookie v10、Simple Cache v5）完全兼容
- mmap 扫描 100MB+ Electron Framework 二进制提取 Chrome 版本字符串，性能可接受（首次 ~1s，后续 OS 缓存）
- `sessionKey` cookie 不存在——账号被封后 cookie 被清除，非版本变更所致
- macOS 版本 `26.4.1`（系统报告的 Darwin 25.4.0 有差异——Info.plist 显示 macOS 26.x，可能是 beta）

**结论 / 下一步**：
- 数据结构无破坏性变更，v0.2.0 代码兼容最新 Claude Desktop + Claude Code
- 动态 UA 检测已实现，后续 Claude Desktop 更新不会导致 UA 过时
- **下一步**：项目拷到 Windows（小桃子），验证 Mode A + 动态 UA 在 Win 上工作
  - 小桃子账号在线，可完整测试 cookie 解密 + API 抓取 + 文件下载
  - 验证 `detect_claude_desktop_info()` 在 Win 上的 pywin32/PowerShell 路径
  - 见 `docs/handoff.md` 详细交接清单

---

## 2026-05-17 02:20~02:50 · 小桃子 Windows · Windows Mode A 验证 + UA 检测修复

**环境**：
- Windows 11 Pro (Build 26200)
- Python 3.12.10 + venv
- Claude Desktop 1.7196.0（Microsoft Store 安装，Electron 41.5.0）
- Claude Code CLI 2.1.143
- 小桃子账号在线，21 条对话
- 项目从移动硬盘拷入 `D:\claudeDataBackup`

**做了什么**：
1. 读交接文档（CLAUDE.md / STATUS.md / testing-log.md / docs/handoff.md），了解最新进展
2. Smoke test：8/8 通过
3. **UA 检测 Bug 修复**：
   - 发现 `_detect_win_claude_info()` 返回全 None——只搜 `%LOCALAPPDATA%/Programs/Claude/` 和 `%ProgramFiles%/Claude/`，不搜 `WindowsApps`
   - 新增 `_find_win_claude_exe()`：标准路径遍历 + `Get-AppxPackage` PowerShell 兜底
   - 重写 `_detect_win_claude_info()`：Electron 版本从 `app/version` 文件读取（41.5.0），Chrome 从映射表推导（146.0.7680.188）
   - 删除无法工作的 chrome.dll PE header 解析代码
   - 验证：`claude_version=1.7196.0, electron_version=41.5.0, chrome_version=146.0.7680.188` ✓
   - User-Agent：`claudeai/1.7196.0 Chrome/146.0.7680.188 Electron/41.5.0` ✓
4. **Mode A 增量备份**：
   - Cookie 解密成功 → 131 字符 sessionKey
   - API 抓取：21 条对话（4 新增 + 2 更新）
   - 文件下载：1 个 .jpg（74 KB），API `/files/` 端点正常
5. **全模式 A+B+C 增量备份**：
   - Mode A：ok（0 新增，2 更新）
   - Mode B：ok（3 条缓存，与旧版一致）
   - Mode C：ok（7 个 real session）
6. HTML 查看器：32 条对话 / 16.9 MB，生成正常
7. 更新 STATUS.md（标记 Windows Mode A 验证完成）
8. 追加 testing-log.md（本条目）

**观察到的现象**：
- Claude Desktop 从上次的 1.5354.0 又更新到了 1.7196.0（两周内 3 次更新：1.4758→1.5354→1.7196），所有更新均向后兼容
- WindowsApps 目录根级文件有特殊 ACL（`version`、`claude.exe`、`*.dll` 直接读取失败），但 `app\` 子目录下文件可正常读写
- `Get-AppxPackage` 是定位 Store 版 Claude 最可靠的方式，Path.glob 可能因 ACL 失败
- Electron 41.5.0 的 Chromium 版本在映射表中查 "41.0.0" 得到 146.0.7680.188，前缀匹配机制工作正常
- Mode B 召回率仍然 ~21%，UWP 版缓存 LRU 淘汰策略不变

**结论 / 下一步**：
- **Windows Mode A 验证完成**：cookie 解密 + UA 检测 + API 抓取 + 文件下载 + 全模式全通过
- Claude Desktop 1.7196.0 与 v0.3.0-dev 代码完全兼容
- `paths.py` 的 UA 检测修复仅在 Win 端（`_find_win_claude_exe()` + 重写的 `_detect_win_claude_info()`），不影响 Mac 端
- 下一步：commit，拷回 Mac 继续 v0.3.0 自动备份开发

---

## 2026-05-18 03:20~03:45 · 小桃子 Windows · v0.3.0 Windows 全功能验证 + 4 个 bug 修复

**环境**：
- Windows 11 Pro (Build 26200)
- Python 3.12.10 + venv（重建，指向 D:\claudeDataBackup）
- Claude Desktop 1.7196.0（Microsoft Store，Electron 41.5.0）
- 小桃子账号在线，22 条对话
- 项目从 Mac 同步最新 v0.3.0-dev 代码

**做了什么**：

### 环境 + 基础验证
1. venv 重建（Mac 路径 → Windows），pip install -e ".[dev]"
2. Smoke test：8/8 通过
3. 动态 UA 检测：`claudeai/1.7196.0 Chrome/146.0.7680.188 Electron/41.5.0` ✓
4. Scheduler config 默认值正确

### Mode A + 全模式验证
5. Mode A 增量备份：22 条对话，2 新增 + 3 更新 ✓
6. 全模式 A+B+C：A=3 + B=3 + C=9，HTML 37 条 19 MB ✓

### CLI schedule 命令
7. `--schedule status`：平台/daemon/触发配置显示正确 ✓
8. `--schedule install`：首次尝试用 schtasks ONLOGON → **失败（拒绝访问）**
9. `--schedule run`：完整备份流程执行成功（daemon 日志确认）✓
10. `--schedule uninstall`：daemon 停止 ✓

### Bug 修复（4 个）

**Bug 1：Task Scheduler ONLOGON 需要管理员权限**
- 现象：`schtasks /Create /SC ONLOGON /RL LIMITED` 在 Win11 返回"拒绝访问"
- 原因：Windows 11 即使 LIMITED 权限，ONLOGON 触发器也需要管理员
- 修复：`scheduler_win.py` 完全重写，改用 `HKCU\Run` 注册表键（不需要管理员）
- 验证：install/uninstall/status 全部正常

**Bug 2：PyInstaller bootloader 重新打开 GUI 窗口**
- 现象：daemon 启动后，exe 重复打开多个 GUI 窗口
- 原因：打包后 `sys.executable` 指向 ClaudeDataBackup.exe（bootloader），不是 Python 解释器。daemon 子进程执行 bootloader 时忽略 `run_daemon.py` 参数，走默认入口重新打开 GUI
- 修复：`scheduler_win.py` 新增 `_find_real_python()` — 搜索 .venv、系统 PATH 中的真实 Python，跳过 bootloader 路径
- macOS 对照：`scheduler_mac.py` 的 `_daemon_python()` 有同样逻辑（`.app/Contents/MacOS/` 检测），Windows 用 `_internal` / `dist` 检测

**Bug 3：daemon 脚本路径在打包后失效**
- 现象：daemon 启动路径 `C:\Users\...\Temp\run_daemon.py`（不存在）
- 原因：`__file__` 在 PyInstaller 包内指向临时目录，`run_daemon.py` 未打包进 exe
- 修复：改为 `python -c "from claude_data_backup.autobackup_daemon import run_daemon; run_daemon()"` 直接调用，不需要外部脚本文件

**Bug 4：daemon 黑色控制台窗口**
- 现象：daemon 进程显示一个黑色无内容的控制台窗口
- 原因：`python.exe` 创建控制台窗口
- 修复：优先使用 `pythonw.exe`（无窗口 Python 解释器）

**附带修复**：
- `autobackup_daemon.py`：`_pid_exists()` 在 Windows 上改用 `ctypes.windll.kernel32.OpenProcess`（`os.kill(pid, 0)` 在 Windows 不可靠）
- `autobackup_daemon.py`：`signal.signal(SIGTERM)` 添加 `threading.current_thread()` 检查（非主线程不能注册信号 handler）
- `gui.py`：`_poll_auto_status()` 每次轮询主动调用 `is_daemon_running()` 检查进程，不依赖缓存状态
- `build-win.bat`：添加 scheduler_win / autobackup_daemon / notifier hidden imports

### 最终验证
- Smoke test：8/8 通过
- Exe 打包：19 MB，启动正常
- Daemon 启动：pythonw.exe 静默运行，无控制台窗口
- GUI 状态：正确显示"● 运行中"
- Git commit：`7aaba01`

**结论 / 下一步**：
- **v0.3.0 Windows 全功能验证 + bug 修复完成**
- exe 可分发：`dist/ClaudeDataBackup.exe`（19 MB）
- 下一步：项目拷回 Mac，推 GitHub，发布 v0.3.0
